...
Piece.prototype.reset = function () {
    ...
    this.disx = 0;
    this.disy = 0;
    this.el.style.width = this.startw + 'px';
    this.el.style.height = this.starth + 'px';
    // 设置x,y为开始值
...
/**
 * 画出钱
 * @param {boolean} isToEnd 是否到达目的地
 */
Piece.prototype.draw = function (isToEnd) {
    if (this.isInStart()) {
        ...
        this.el.style.top = this.y + 'px';
        if (isToEnd) {
            this.el.style.width = this.endw + 'px';
            this.el.style.height = this.endh + 'px';
        }
    }
}
/**
...
Piece.prototype.isInEnd = function () {
    return this.status == 'end' || !this.targetmsg;
}