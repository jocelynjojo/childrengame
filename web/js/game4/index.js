...
// 内容
var con = document.getElementById('js-con4');
/*
* 整个游戏对象
*/
var Game = {
  ...
  */
  init: function (opts) {
    ...
    var _self = this
    // 加载资源图片, 加载完成交互才开始
    var resources = opts.pieceSrc.concat(opts.pressSrc).concat([opts.bgSrc])
  ...
  createObj: function () {
    ...
    for (var i = 0, len = opts.pieceMsgs.length; i < len; i++) {
      ...
      var o = {
        ...
        targetmsg: this.getChangeMsg(tgmsg)
      }
  ...
  getChangeMsg: function (msg) {
    ...
    if (msg.endw && msg.endh) {
      obj.endw = msg.endw * percent;
      obj.endh = msg.endh * percent;
    }
    if (msg.type) {
      obj.type = msg.type;
    }
 ...
 */
    ...
    console.log('end')
    document.getElementById('js-gradewp').style.display = 'block';
    // this.grade.sendMsg();
  },
  ...
  trigger: function (type, extra) {
    if (type == 'touchstart') {
      for (var i = 0, len = this.pieces.length; i < len; i++) {
        ...
        if (extra.tg == piece.el && piece.isInStart() && !this.isEnd()) {
