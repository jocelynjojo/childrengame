...
var Game = {
  ...
  */
  ...
  createObj: function () {
    ...
    for (var i = 0, len = opts.moneyMsgs.length; i < len; i++) {
      var msg = opts.moneyMsgs[i], targetmsg = {};
      if (msg.type == 'coin') {
        targetmsg = opts.containerMsg[0]
      } else {
        targetmsg = opts.containerMsg[1]
      }
      var el = document.createElement('img');
      el.src = opts.moneySrc[i];
      el.className = 'piece2'
      ...
      var o = {
        el: el,
        pel: pel,
        msg: this.getChangeMsg(opts.moneyMsgs[i]),
  ...
  */
  ...
  reset: function () {
    var money = null;
    for (var i = 0, total = this.moneys.length; i < total; i++) {
      this.moneys[i].reset();
    }
  },
  play: function () {
