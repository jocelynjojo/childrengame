...
var Game = {
    ...
    playGame: function (event) {
        ...
        for (var i = 0, allNum = this.opts.levelsMsg.length; i < allNum; i++) {
            var level = this.opts.levelsMsg[i] || { x: 0, y: 0, w: 0, h: 0 };
            if (sx >= level.x && sx <= level.x + level.w && sy >= level.y && sy <= level.y + level.h) {
                location.href = 'game' + (i + 1) + '.html'
                break;
            }
        }
    }
