...
var Game = {
  ...
  */
  ...
  reset: function () {
    ...
    var total = this.upfinish.length = this.pieces.length;
    var piece = null;
    for (var i = 0; i < total; i++) {
      piece = this.pieces[i];
      piece.reset();
      this.upfinish[i] = piece;
    }
  ...
  */
  ...
  showGrade: function () {
    ...
    if (percent > starNum - 1) {
      gradewp.className = gradewp.className + ' js-star1';
    } else {
      gradewp.className = gradewp.className + ' js-star' + (starNum - Math.floor(percent));
    }
